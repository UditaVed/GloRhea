...
const CardFAQ = ({ Query, Quote, Analysis, BgQuoteColor,Img,Key }) => {
  ...
  return (
    ...
    variants={fadeIn('left', 'spring', index * 0.5, 0.75)}
      tabIndex={0}
      className="collapse collapse-arrow border border-base-300 bg-[#F1F1F1] rounded-box  "
    >
    <div className="flex gap-2 h-[15vh]">
    <div class=" w-[15%] p-4 rounded-3xl overflow-hidden  ">
            <img src={Img} alt="" className="w-full h-full  rounded-3xl object-cover"/>
          </div>
    <div className="w-[80%]  align-center flex items-centerOccaecat labore est proident elit eu proident.">
    <div className="collapse-title text-sm sm:text-xl font-medium items-center">
      ...
      <div className="collapse-content bg-white m-4 pt-2 rounded-lg">
        <p> {Analysis}</p>
      </div>
    </motion.div>
  );
